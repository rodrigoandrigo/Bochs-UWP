...
extern "C" {
  ...
  ssize_t bx_uwp_file_write(int handle, const void *buf, size_t count);
}
...
#if defined(BX_UWP_CORE_LIBRARY)
    BX_FD_THIS s.media[i].uwp_file          = 0;
    BX_FD_THIS s.media[i].uwp_handle        = -1;
    BX_FD_THIS s.media[i].uwp_size          = 0;
#endif
...
void bx_floppy_ctrl_c::floppy_xfer(Bit8u drive, Bit32u offset, Bit8u *buffer, Bit32u bytes, Bit8u direction)
{
...
#if defined(BX_UWP_CORE_LIBRARY)
    } else if (BX_FD_THIS s.media[drive].uwp_file) {
      ret = (int)bx_uwp_file_seek(BX_FD_THIS s.media[drive].uwp_handle, offset, SEEK_SET);
#endif
    } else {
  ...
  if (direction == FROM_FLOPPY) {
    if (BX_FD_THIS s.media[drive].vvfat_floppy) {
      ...
      ret = (int)bx_uwp_file_read(BX_FD_THIS s.media[drive].uwp_handle, buffer, bytes);
#endif
#if BX_WITH_MACOS
    ...
    if (BX_FD_THIS s.media[drive].vvfat_floppy) {
      ...
      ret = (int)bx_uwp_file_write(BX_FD_THIS s.media[drive].uwp_handle, buffer, bytes);
#endif
#if BX_WITH_MACOS
    } else if (!strcmp(SIM->get_param_string(pname)->getptr(), SuperDrive)) {
...
bool bx_floppy_ctrl_c::evaluate_media(Bit8u devtype, Bit8u type, const char *path, floppy_t *media)
{
  ...
  if (bx_is_uwp_file_uri(path)) {
    Bit64u uwp_size = 0;
    unsigned uwp_flags = media->write_protected ? BX_RDONLY : BX_RDWR;
    media->uwp_handle = bx_uwp_file_open(path, uwp_flags, &uwp_size);
    if (!media->write_protected && (media->uwp_handle < 0)) {
      BX_INFO(("tried to open UWP floppy '%s' read/write", path));
      media->write_protected = 1;
      media->uwp_handle = bx_uwp_file_open(path, BX_RDONLY, &uwp_size);
    }
    if (media->uwp_handle < 0) {
      BX_INFO(("tried to open UWP floppy '%s' read only", path));
      media->type = type;
      return 0;
    }
    media->uwp_file = 1;
    media->uwp_size = uwp_size;
    media->fd = media->uwp_handle;
  } else
#endif
  ...
  if ((isalpha(path[0])) && (path[1] == ':') && (strlen(path) == 2)) {
    raw_floppy = 1;
#if defined(BX_UWP_CORE_LIBRARY)
    sprintf_s(sTemp, sizeof(sTemp), "\\\\.\\%s", path);
#else
...
#if defined(BX_UWP_CORE_LIBRARY)
  else if (media->uwp_file) {
    memset(&stat_buf, 0, sizeof(stat_buf));
    stat_buf.st_mode = S_IFREG;
    stat_buf.st_size = (off_t)media->uwp_size;
    ret = 0;
  }
#endif
  else
...
void bx_floppy_ctrl_c::close_media(floppy_t *media)
{
  if (media->fd >= 0) {
    if (media->vvfat_floppy) {
      ...
      bx_uwp_file_close(media->uwp_handle);
      media->uwp_file = 0;
      media->uwp_handle = -1;
      media->uwp_size = 0;
#endif
