...
#if defined(BX_UWP_CORE_LIBRARY)
  } else if (!strcmp(image_mode, "uwp")) {
    hdimage = new uwp_image_t();
#endif
  } else {
...
#if defined(BX_UWP_CORE_LIBRARY) && !defined(BXIMAGE)
extern "C" {
...
int uwp_image_t::open(const char* pathname, int flags)
{
  handle = bx_uwp_file_open(pathname, (unsigned)flags, &hd_size);
  if (handle < 0) {
    return -1;
  }
  BX_INFO(("uwp hd_size: " FMT_LL "u", hd_size));
  if (hd_size <= 0) BX_PANIC(("size of UWP disk image not detected / invalid"));
  if ((hd_size % sect_size) != 0) {
    BX_PANIC(("size of UWP disk image must be multiple of %d bytes", sect_size));
  }
  position = 0;
  return handle;
}

void uwp_image_t::close()
{
  if (handle >= 0) {
    bx_uwp_file_close(handle);
    handle = -1;
  }
}

Bit64s uwp_image_t::lseek(Bit64s offset, int whence)
{
  position = bx_uwp_file_seek(handle, offset, whence);
  return position;
}

ssize_t uwp_image_t::read(void* buf, size_t count)
{
  ssize_t ret = bx_uwp_file_read(handle, buf, count);
  if (ret > 0) {
    position += ret;
  }
  return ret;
...
ssize_t uwp_image_t::write(const void* buf, size_t count)
{
  ssize_t ret = bx_uwp_file_write(handle, buf, count);
  if (ret > 0) {
    position += ret;
  }
  return ret;
