...
#if defined(BX_WITH_UWP_DX)
extern "C" void bochs_core_uwp_report_log(int level, const char *prefix, const char *message);
#endif
...
void iofunctions::out(int level, const char *prefix, const char *fmt, va_list ap)
{
  ...
  fflush(logfd);
#if defined(BX_WITH_UWP_DX)
  bochs_core_uwp_report_log(level, msgpfx, msg);
#endif
