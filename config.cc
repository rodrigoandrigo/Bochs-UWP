...
static int get_floppy_type_from_size(Bit64u size)
{
    switch (size) {
      case 163840:
...
#if defined(BX_UWP_CORE_LIBRARY)
  else if (bx_is_uwp_file_uri(filename)) {
    Bit64u uwp_size = 0;
    int handle = bx_uwp_file_open(filename, 0, &uwp_size);
    if (handle < 0) {
      return BX_FLOPPY_NONE;
    }
    bx_uwp_file_close(handle);
    return get_floppy_type_from_size(uwp_size);
  }
#endif
  else if (stat(filename, &stat_buf)) {
    return BX_FLOPPY_NONE;
  } else {
    return get_floppy_type_from_size((Bit64u)stat_buf.st_size);
  }
}
